/**
 * Listado de Receta Detalle. Usa sheets-listar-receta-detalle.config.js
 */
(function () {
    var STORAGE_EDIT = "costosEditRecordRecetaDetalle";
    var STORAGE_FILTRO = "recetaDetalleFiltroValor";
    var ACCIONES = window.RECETA_DETALLE_ACCIONES || {};
    var LISTAR_URL = (ACCIONES.listar && ACCIONES.listar.url) ? ACCIONES.listar.url : "receta-detalle.html";
    var CREAR_URL = (ACCIONES.crear && ACCIONES.crear.url) ? ACCIONES.crear.url : "crear-receta-detalle.html";
    var EDITAR_URL = (ACCIONES.editar && ACCIONES.editar.url) ? ACCIONES.editar.url : "editar-receta-detalle.html";
    var VER_URL = (ACCIONES.ver && ACCIONES.ver.url) ? ACCIONES.ver.url : "ver-receta-detalle.html";
    ...
    function renderList(container, sheetConfig, headers, rows, filterText, valorFiltroColumna, indiceColumnaFiltro) {
        ...
        var clavePrimaria = sheetConfig.clavePrimaria || ["IDReceta-Base-Detalle"];
        ...
        var nameColIdx = headers.indexOf("Nombre-Insumo");
        if (nameColIdx < 0) nameColIdx = findColumnIndex(headers, ["nombreinsumo", "nombre-insumo", "descripcion"]);
        ...
        var html = '<div class="costos-cards costos-cards-receta-detalle">';
        visible.forEach(function (row) {
            ...
            html += "<div class=\"costos-card costos-card-receta-detalle\">";
        ...
        var countEl = document.getElementById("receta-detalle-count");
    ...
    function run() {
        var container = document.getElementById("datos-receta-detalle");
        var filterInput = document.getElementById("filter-receta-detalle");
        var btnNuevo = document.getElementById("btn-nuevo-receta-detalle");
        if (!container) return;
        ...
        loadConfig().then(function (sheetConfig) {
            ...
            if (!nombreHoja) { showMessage(container, "Configuración incorrecta: no se especificó la hoja (Tabla-Receta-Base-Detalle)."); return; }
            var columnas = sheetConfig.columnas || [];
            ...
            var filtrarWrap = document.getElementById("receta-detalle-filtrar-wrap");
            var filtrarSelect = document.getElementById("filtrar-valores-receta-detalle");
            ...
            return fetch(url, { cache: "no-store" }).then(function (res) { return res.json(); }).then(function (json) {
                ...
                function getValorFiltroActual() { var sel = document.getElementById("filtrar-valores-receta-detalle"); return sel ? (sel.value || "") : ""; }
                function redraw() {
